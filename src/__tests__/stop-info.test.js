...
describe('StopInfo', () => {
  it('displays name and direction', () => {
    const { container } = render(
      <StopInfo name={stopName} direction={direction} />
    );
  ...
  it('displays cleaned name if it contains @', () => {
    const { container } = render(
      <StopInfo name="One @ Two" direction={direction} />
    );
  ...
  it('passes color to color pill', () => {
    ...
    expect(getByTestId('color-pill')).toHaveStyleRule(
      'background-color',
      '#red'
    );
  });
  ...
  it('displays only first letter if props.isCompact goes from false to true', () => {
    const { container, rerender } = render(
      <StopInfo name="Harvard" isCompact={false} />
    );
